...
export async function POST(request: Request) {
  ...
  try {
    body = await readJsonBody<ProofBody>(request, 16 * 1024);
  } catch (error) {
  ...
  if (latest?.status === "approved")
    return NextResponse.json(
      {
        error:
          "The customer-approved proof is locked. Do not create another proof version unless the approved proof is deliberately reopened.",
